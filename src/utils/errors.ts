 ...
 */
export function displayError(type: ErrorType, ...args: unknown[]): void {
 ...
 */
export function displayContextWarning(currentSize: number, maxSize: number): void {
  const version = packageJson.version;

  console.log(
    `agentx v${version} | ${colors.cyan(loadConfig().provider)} | ${colors.yellow('warning')}`
  );
  console.log('');
  console.log(
