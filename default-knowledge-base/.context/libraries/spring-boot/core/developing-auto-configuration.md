---
title: "Developing Auto-configuration"
...
You can browse the source code of [`spring-boot-autoconfigure`] to see the core `@AutoConfiguration` classes that Spring provides (see the /core/spring-boot-autoconfigure/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports[`META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`] file).
...
> **Note:** Auto-configurations must be loaded *only* by being named in the imports file.
...
> **Note:** The `AutoConfiguration.imports` file should also be updated to *only* reference the replacement class.
...
The `@ConditionalOnExpression` annotation lets configuration be included based on the result of a /core/expressions.html[SpEL expression].
...
```java
// Example: MyConditionEvaluationReportingTests
```
