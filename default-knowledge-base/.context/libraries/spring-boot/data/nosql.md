...
Additionally, [Spring Boot for Apache Geode] provides #geode-repositories[auto-configuration for Apache Geode].
You can make use of the other projects, but you must configure them yourself.
See the appropriate reference documentation at .
...
If you have defined your own javadoc:/com.mongodb.client.MongoClient[], it will be used to auto-configure a suitable `MongoDatabaseFactory`.

The auto-configured javadoc:/com.mongodb.client.MongoClient[] is created using a javadoc:/com.mongodb.MongoClientSettings[] bean.
If you have defined your own javadoc:/com.mongodb.MongoClientSettings[], it will be used without modification and the `spring.data.mongodb` properties will be ignored.
Otherwise a javadoc:/com.mongodb.MongoClientSettings[] will be auto-configured and will have the `spring.data.mongodb` properties applied to it.
In either case, you can declare one or more `MongoClientSettingsBuilderCustomizer` beans to fine-tune the javadoc:/com.mongodb.MongoClientSettings[] configuration.
Each will be called in order with the javadoc:/com.mongodb.MongoClientSettings$Builder[] that is used to build the javadoc:/com.mongodb.MongoClientSettings[].
...
The auto-configured javadoc:/com.mongodb.client.MongoClient[] can be configured to use SSL for communication with the server by setting the properties as shown in this example:
...
Custom SSL trust material can be configured in an SSL bundle and applied to the javadoc:/com.mongodb.client.MongoClient[] as shown in this example:
...
> **Tip:** If you do not use Spring Data MongoDB, you can inject a javadoc:/com.mongodb.client.MongoClient[] bean instead of using `MongoDatabaseFactory`.
If you want to take complete control of establishing the MongoDB connection, you can also declare your own `MongoDatabaseFactory` or javadoc:/com.mongodb.client.MongoClient[] bean.
...
```java
// Example: MyCouchbaseConfiguration
```

## LDAP

[LDAP](https://en.wikipedia.org/wiki/Lightweight*Directory*Access_Protocol) (Lightweight Directory Access Protocol) is an open, vendor-neutral, industry standard application protocol for accessing and maintaining distributed directory information services over an IP network.
...
> **Tip:** For complete details of Spring Data LDAP, see the [reference documentation].
