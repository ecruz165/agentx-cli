...
If you use the `spring-boot-actuator` (see the  section), there is also a `conditions` endpoint that renders the report in JSON.
...
See  in the "`Spring Boot Features`" section for a complete list.
...
```java
// Example: MyEnvironmentPostProcessor
```
