---
title: "How-to: Properties and Configuration"
...
> **Note:** The `useDefaultDelimiters` property is important if you use standard Spring placeholders (such as `$\`) in your configuration.
...
```java
// Example: application/MyApplication
```

Or to `sources(...)` method of a `SpringApplicationBuilder`:

```java
// Example: builder/MyApplication
```
...
* `spring.config.name` (configprop:spring.config.name[format=envvar]): Defaults to `application` as the root of the file name.
* `spring.config.location` (configprop:spring.config.location[format=envvar]): The file to load (such as a classpath resource or a URL).
...
The Spring `Environment` has an API for this, but you would normally set a System property (`spring.profiles.active`) or an OS environment variable (configprop:spring.profiles.active[format=envvar]).
...
By default, the name of the default profile is `default`, but it could be changed using a System property (`spring.profiles.default`) or an OS environment variable (configprop:spring.profiles.default[format=envvar]).
