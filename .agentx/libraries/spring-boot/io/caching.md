...
For more details, check the /integration/cache.html[relevant section] of the Spring Framework reference documentation.
...
> **Tip:** Avoid adding `@EnableCaching` to the main method's application class.
...
> **Tip:** It is also possible to transparently /integration/cache/annotations.html#cache-annotations-put[update] or /integration/cache/annotations.html#cache-annotations-evict[evict] data from the cache.
...
> **Tip:** If the `CacheManager` is auto-configured by Spring Boot, it is possible to *force* a particular cache provider by setting the `spring.cache.type` property.
...
```java
// Example: MyCacheManagerConfiguration
```
...
[JCache](https://jcp.org/en/jsr/detail?id=107) is bootstrapped through the presence of a `CachingProvider` on the classpath (that is, a JSR-107 compliant caching library exists on the classpath), and the `JCacheCacheManager` is provided by the `spring-boot-starter-cache` starter.
...
> **Tip:** Spring Boot has general support for Hazelcast.
...
> **Tip:** If a standard `CacheManager` bean is defined, it is wrapped automatically in an `CacheManager` implementation that the abstraction expects.
...
```java
// Example: MyCouchbaseCacheManagerConfiguration
```
...
```java
// Example: MyRedisCacheManagerConfiguration
```
...
```java
// Example: MyCache2kDefaultsConfiguration
```
