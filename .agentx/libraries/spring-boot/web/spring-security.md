...
Additional information can be found in the /servlet/authorization/method-security.html[Spring Security Reference Guide].
...
* A `UserDetailsService` (or `ReactiveUserDetailsService` in case of a WebFlux application) bean with in-memory store and a single user with a generated password (see javadoc:org.springframework.boot.security.autoconfigure.SecurityProperties$User[] for the properties of the user).
...
```java
// Example: MyWebFluxSecurityConfiguration
```
...
```java
// Example: MyOAuthClientConfiguration
```
...
Additional information can be found in the /servlet/oauth2/authorization-server/getting-started.html[Getting Started] chapter of Spring Security Reference Documentation.
...
```java
// Example: MySamlRelyingPartyConfiguration
```
