...
> **Caution:** Devtools might cause classloading issues, in particular in multi-module projects.
 explains how to diagnose and solve them.
...
As described in the  section, restart functionality is implemented by using two classloaders.
...
> **Note:** If you do not want property defaults to be applied you can set `spring.devtools.add-properties` to `false` in your `application.properties`.
...
As described earlier in the  section, restart functionality is implemented by using two classloaders.
...
> **Warning:** To trigger LiveReload when a file changes,  must be enabled.
...
To customize this location, set the `SPRING*DEVTOOLS*HOME` environment variable or the `spring.devtools.home` system property.
...
If you observe such problems constantly, try increasing the `spring.devtools.restart.poll-interval` and `spring.devtools.restart.quiet-period` parameters to the values that fit your development environment.
See the  section for configuring these properties.
